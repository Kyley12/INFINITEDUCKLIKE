...
namespace FronkonGames.Retro.OldComputers
{
  ...
  public sealed partial class OldComputers
  {
    ...
    private static readonly OldComputers[] NoEffects = new OldComputers[0];
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    public static OldComputers Instance
    {
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          {
            if (scriptableRendererData.rendererFeatures[i] is OldComputers)
              return scriptableRendererData.rendererFeatures[i] as OldComputers;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    private static OldComputers[] Instances
    {
      get
      {
          ...
          for (int i = 0; i < rendererDataList.Length; ++i)
          {
            if (rendererDataList[i] != null && rendererDataList[i].rendererFeatures.Count > 0)
              foreach (var feature in rendererDataList[i].rendererFeatures)
                if (feature is OldComputers)
                  effects.Add(feature as OldComputers);
          }
